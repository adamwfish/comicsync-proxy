...
app.post("/schedule", async (req, res) => {
  ...
  const timer = setTimeout(async () => {
    try {
      ...
      if (ok) { console.log(`[schedule] Auto-published product ${productId} ✓`); }
      else { console.error(`[schedule] All publish attempts failed for ${productId}`); }
    } catch (err) {
...
app.post("/upload-images", upload.fields([{ name: "front", maxCount: 1 }, { name: "back", maxCount: 1 }]), async (req, res) => {
  ...
  for (const { file, label } of files) {
    try {
      ...
      if (!imgRes.ok) { results.push({ label, success: false, error: imgData.message || JSON.stringify(imgData) }); }
      else { results.push({ label, success: true, imageId: imgData.id }); }
    } catch (e) {
      ...
      for (const p of data.products || []) {
        ...
        if (matchesPage) {
          ...
          products.push({
            ...
            isVisible: p.isVisible, thumbnail: p.images?.[0]?.url || "", createdOn: p.createdOn || "",
          });
        }
      }
...
// ─── SEARCH PROXY ─────────────────────────────────────────────────────────────
// CORS bypass for comic search API calls from the frontend
